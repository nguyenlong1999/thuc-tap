...
class BiddingPackage(models.Model):
    ...
    status = fields.Selection([
        ...
        ("1", "đã duyệt"),
        ("2", "chờ xác nhận"),
    ], integer="Status", required=True, default="0")
    ...
    to_address = fields.Date(date="To Address", required=True)
    is_publish = fields.Boolean(char="Is public", required=True, default=True)
    is_auto = fields.Char(string="Auto", required=True)
    publish_time = fields.Integer(string="Public time", required=True)
